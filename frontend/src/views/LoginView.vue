      ...
      <form @submit.prevent="handleLogin" class="flex flex-col gap-6">
        ...
        <div class="flex flex-col gap-2">
          ...
          <div class="relative">
            ...
            <input v-model="email" type="email" required class="w-full bg-[#11062b] border border-[#251052] rounded-xl py-3 pl-12 pr-4 text-white focus:border-violet-500 outline-none transition-all" placeholder="nome@email.com"/>
        ...
        <div class="flex flex-col gap-2">
          ...
          <div class="relative">
            ...
            <input v-model="password" :type="showPassword ? 'text' : 'password'" required class="w-full bg-[#11062b] border border-[#251052] rounded-xl py-3 pl-12 pr-12 text-white focus:border-violet-500 outline-none transition-all" placeholder="••••••••"/>
            
            <button type="button" @click="showPassword = !showPassword" class="absolute right-4 top-1/2 -translate-y-1/2 text-violet-400 hover:text-white transition-colors focus:outline-none">
        ...
        <Button 
          label="Entrar no Sistema" type="submit" :loading="loading" class="!bg-violet-600 !border-none !rounded-xl !py-4 font-bold text-lg mt-2 shadow-lg shadow-violet-600/20">
        </Button>
      </form>

      <div class="text-center mt-10">
        <p class="text-violet-300">Não tem uma conta?<router-link to="/register" class="text-violet-400 font-bold hover:text-white transition-colors"> Cadastre-se</router-link></p>
      </div>
