    ...
    <nav class="bg-[#1a0b3b]/50 backdrop-blur-md border-b border-[#251052] px-6 py-3 flex justify-between items-center shadow-lg sticky top-0 z-50">
      ...
      <div class="flex items-center gap-6">
        ...
        <div class="flex items-center gap-3 bg-[#11062b]/50 p-1 pr-4 rounded-full border border-[#251052]">
          ...
          <div class="text-left">
            <p class="text-[12px] text-violet-400 font-black tracking-tighter leading-none mb-1">Olá,</p>
         ...
         <form @submit.prevent="submitProduct" class="flex flex-col gap-6 mt-4">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="flex flex-col gap-2">
              ...
              <input :value="displayPrice" @input="onPriceInput" type="text" required class="bg-[#11062b] border border-[#251052] rounded-xl p-3 text-white focus:border-violet-500 outline-none" placeholder="R$ 0,00"/>
            </div>
          ...
          <div class="flex flex-col gap-2">
            ...
            <textarea v-model="form.description" maxlength="200" rows="3" class="bg-[#11062b] border border-[#251052] rounded-xl p-3 text-white focus:border-violet-500 outline-none" placeholder="Conte mais...">
            </textarea>
...
// Variáveis de utilidade global da tela
const router = useRouter();
const toast = useToast();
const confirmService = useConfirm(); 

// variáveis de controle de dados que aparecem na tela 
const userName = ref('...');
...
// variáveis de controle do modal e formulário
...
// armazena os dados digitados no formulário do modal
const form = ref({ 
  name: '', 
  description: '', 
  price: '', 
...
// formatação do número em dinheiro (R$)
...
// formata o preço imediatamente enquanto o usuário digita
...
// insere a opção "Todas as Categorias" no filtro
const categoryOptions = computed(() => {
  return [{ id: null, name: 'Todas as Categorias' }, ...categories.value];
});

// calcula a data de hoje para bloquear seleções no passado na data de validade
const today = computed(() => new Date().toISOString().split('T')[0]);

// verifica automaticamente se a categoria escolhida é "Alimentos"
...
// força a ativação do checkbox de validade se for "Alimentos"
...
// puxa a lista de produtos do Backend
...
// evita que a pesquisa dispare requisições a cada letra digitada
let searchTimer: any;
const handleSearch = () => {
...
// função para preencher o formulário e abrir o modal de Edição
...
// limpa o campo data caso o usuário desmarque o checkbox
watch(hasExpiration, (val) => {
  if (!val) form.value.expiration_date = '';
});

// pega o arquivo de imagem do input e gera um preview na tela
...
// envia o formulário para o Backend (salvar ou atualizar)
const submitProduct = async () => {
  try {
    ...
    let url = `/products`;
    // se for edição, ajusta a URL e adiciona o método PUT
    if (editingId.value) {
      url += `/${editingId.value}`;
...
// aciona o alerta de deletar e deleta o produto do banco
...
// ação disparada quando a tela carrega (busca nome salvo e inicializa listas)
onMounted(() => {
...
// Função para excluir o cadastro do usuário atual e seus dados
const deleteAccount = () => {
  confirmService.require({
...
/* Centraliza os títulos das colunas na tabela */
...
/* linhas normais da tabela deixando transparentes */
...
/* aplica borda em baixo de cada célula na tabela */
...
/* título e conteúdo de dentro do Modal */
.p-dialog-header { 
  background: transparent !important; 
  color: white !important; 
  border-bottom: 1px solid #251052 !important; 
...
/* ícone de calendário nos inputs de data para ficar branco */
::-webkit-calendar-picker-indicator { 
  filter: invert(1); 
  cursor: pointer; 
}

/* animação para itens surgirem deslizando de cima para baixo */
@keyframes fadein { 
  from { 
    opacity: 0; 
