  ...
  <div class="min-h-screen bg-[#11062b] flex items-center justify-center font-sans p-6">
    ...
    <div class="w-full max-w-md bg-[#1a0b3b]/50 backdrop-blur-xl border border-[#251052] rounded-3xl p-10 shadow-2xl shadow-black/50">
      ...
      <form @submit.prevent="handleRegister" class="flex flex-col gap-5">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div class="flex flex-col gap-2 text-left">
            ...
            <div class="relative">
              ...
              <input v-model="form.password" :type="showPassword ? 'text' : 'password'" required class="w-full bg-[#11062b] border border-[#251052] rounded-xl py-3 pl-11 pr-11 text-white focus:border-violet-500 outline-none transition-all" placeholder="••••••••"/>

              <button type="button" @click="showPassword = !showPassword" class="absolute right-3 top-1/2 -translate-y-1/2 text-violet-400 hover:text-white transition-colors focus:outline-none">
          ...
          <div class="flex flex-col gap-2 text-left">
            ...
            <div class="relative">
              <input v-model="form.password_confirmation" :type="showPassword ? 'text' : 'password'" required class="w-full bg-[#11062b] border border-[#251052] rounded-xl py-3 px-4 text-white focus:border-violet-500 outline-none transition-all" placeholder="••••••••"/>
            </div>
        ...
        <Button 
          label="Cadastrar Conta" type="submit" :loading="loading" raised class="!bg-violet-600 !border-none !rounded-xl !py-4 font-bold text-lg mt-4 shadow-lg shadow-violet-600/20">
        </Button>
      </form>

      <div class="text-center mt-8">
        <p class="text-violet-300">Já tem uma conta? <router-link to="/login" class="text-violet-400 font-bold hover:text-white transition-colors">Fazer Login</router-link></p>
      </div>
